...
- **VPN & Remote Access files** (OpenVPN · WireGuard · AmneziaWG · SSTP · GRE · L2TP/IPsec · PPTP · IKEv2 · Cisco AnyConnect) — download/copy `.ovpn` profiles and `.conf` profiles (WireGuard & AmneziaWG) with direct download and quick connect, masked credential cards with MTU/TTL/DNS for SSTP and GRE, fed by the panel's `/info` endpoint; IKEv2 and AnyConnect support password and certificate auth. Lazy-loaded.
